...
def decode(input_length, tagset, score):
    ...
    # viterbi[m][tag:k] = best local score up to sentence position m given tag k
    viterbi = [dict() for x in range(input_length)] # dp array initialized with dictionaries in each index
    backtrack = [dict() for x in range(input_length)] # backtrack array stores parent pointers with a dictionary (tag: parent_tag)

    # initialize position 1 of the viterbi array with scores between all tags and the start symbol
    for tag in tagset:
        viterbi[1][tag] = score(tag, '<START>', 1)

    # iterates from m=2->M, where M is the number of tokens in the input.
    # m=0 is <START>, m=M+1 is <STOP>, m=1 is first token, m=M is last token
    # M = input_length-2
    for m in range(2, input_length-1):
        for tag in tagset: # iterate over each K tags
            # finding max score and argmax
            max_score = 0
            max_tag = '<temp>' # this should get overwritten
            for tagp in tagset: # finds best score over the current tag k and all tags k'
                local_score = score(tag, tagp, m) + viterbi[m-1][tagp]
                if local_score > max_score:
                    max_score = local_score
                    max_tag = tagp

            viterbi[m][tag] = max_score
            backtrack[m][tag] = max_tag

    # find last tag
    max_score = 0
    max_tag = '<temp>'
    for tag in tagset:
        local_score = score('<STOP>', tag, input_length-1) + viterbi[input_length-2][tag]
        if local_score > max_score:
            max_score = local_score
            max_tag = tag

    # build tag sequence starting from the end
    y = ['<STOP>']
    y.insert(0, max_tag)

    for m in range(input_length-3,0,-1): # m in range M-1 -> 1
        curtag = y[0]
        y.insert(0, backtrack[m+1][curtag])
    y.insert(0, '<START>')
...
def sgd(training_size, epochs, gradient, parameters, training_observer):
    ...
    for e in range(epochs):
        for i in np.random.permutation(training_size):
            parameters.times_plus_equal(-1, gradient(i)) # 1 is the step_size/alpha
        training_observer(e, parameters)
...
def train(data, feature_names, tagset, epochs):
    ...
    def training_observer(i, parameters):
        ...
        write_predictions('ner.dev.out'+str(i), dev_data, parameters, feature_names, tagset)
...
def main_train():
    ...
    # train_data = read_data('ner.train')[0:1] # if you want to train on just one example
...
main_predict('ner.test', 'model.iter6')  # Uncomment to predict on 'dev.ner' using the model 'model.simple' (need to implement 'decode' function)
